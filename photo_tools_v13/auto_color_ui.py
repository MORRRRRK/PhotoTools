...
class CompareView(QWidget):
    ...
    def _image_rect(self):
        source = self.result or self.orig
        if source is None or source.isNull():
            return self.rect()
        from PySide6.QtCore import QSize
        scaled = QSize(source.width(), source.height())
        scaled.scale(self.rect().size(), Qt.KeepAspectRatio)
    ...
    def _update_split(self, x):
        ...
        if rect.width() > 0:
            self.split = max(0.0, min(1.0, (x - rect.x()) / rect.width()))
            self.update()

    def paintEvent(self, event):
        p = QPainter(self)
        p.fillRect(self.rect(), _theme_color("CANVAS", "#0d1014"))
        target = self._image_rect()
        if self.result and not self.result.isNull():
            p.drawPixmap(target, self.result)
        if self.orig and not self.orig.isNull():
            cut = target.x() + int(target.width() * self.split)
            p.save()
            p.setClipRect(target.x(), target.y(), max(0, cut - target.x()), target.height())
            p.drawPixmap(target, self.orig)
            p.restore()
            p.setPen(QPen(QColor("#e8e8ed"), 2))
            p.drawLine(cut, target.y(), cut, target.y() + target.height())
            p.setBrush(_theme_color("ACCENT", "#2563eb"))
            p.setPen(Qt.NoPen)
            p.drawRect(cut - 5, target.center().y() - 12, 10, 24)
        p.end()
