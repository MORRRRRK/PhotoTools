...
from .image_view import theme_color
...
class CropPreview(QWidget):
    ...
    def paintEvent(self, event):
        p = QPainter(self)
        p.fillRect(self.rect(), theme_color("CANVAS", "#0d1014"))
        disp = self._rect()
...
class ImagePipelinePage(QWidget):
    ...
    def _build_ui(self):
        ...
        top.addWidget(title)
        self.page_desc = QLabel("三阶段流水线：几何校正 → 调色增强 → 构图裁剪；输出无损 PNG，不覆盖历史版本。")
