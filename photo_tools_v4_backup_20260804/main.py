...
class PhotoToolsApp(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("PhotoTools V4 — 摄影素材管理工具箱")
    ...
    def _build_ui(self):
        ...
        ctk.CTkLabel(ab, text="关于 PhotoTools V4", font=("", 14, "bold")).pack(anchor="w", pady=5)
        ctk.CTkLabel(ab, text="版本 4.0.0 | 摄影素材管理工具箱").pack(anchor="w")
        ctk.CTkLabel(ab, text="功能: 文件类型筛选 / 照片视频质量评估 / 视频代理生成 / PushPlus 推送").pack(anchor="w")
    ...
    def _t1_preview(self,fp):
        from photo_tools_v4.preview import load_image
