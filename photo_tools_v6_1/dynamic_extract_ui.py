"""dynamic_extract_ui.py - 动态照片提取界面 (V6.1)"""
...
class DynamicExtractTab(ctk.CTkFrame):
    ...
    def _build_ui(self):
        ...
        ctk.CTkLabel(header, text="文件夹名称", font=F(12, True), width=270, anchor="w").grid(
            row=0, column=0, sticky="w", padx=(4, 4))
        ctk.CTkLabel(header, text="完整路径", font=F(12, True), anchor="w").grid(
            row=0, column=1, sticky="ew", padx=4)
        self.list_body = ctk.CTkScrollableFrame(list_frame, height=180)
    ...
    def _render_list(self):
        ...
        for item in self.folder_items:
            ...
            ctk.CTkCheckBox(row, text=name, variable=item["var"], font=F(12),
                            width=270).grid(row=0, column=0, sticky="w", padx=(6, 4))
            ctk.CTkLabel(row, text=item["path"], font=F(11), anchor="w").grid(
                row=0, column=1, sticky="ew", padx=4)
            jpgs, mp4s = list_media(item["path"])
            names = [os.path.basename(f) for f in (jpgs + mp4s)]
            if not names:
                file_text = "未找到 JPG / MP4 文件"
            else:
                file_text = " / ".join(names) + f"（JPG {len(jpgs)}，MP4 {len(mp4s)}）"
            ctk.CTkLabel(row, text=file_text, font=F(11), anchor="w",
                         text_color="#7f8c8d").grid(row=1, column=0, columnspan=2,
                                                    sticky="w", padx=(12, 4), pady=(0, 2))
            self.row_widgets.append(row)
        self.count_lb.configure(text=f"共 {len(self.folder_items)} 个文件夹，已勾选 {self._selected_count()} 个")

    def _collect_dynamic_targets(self, selected):
        targets = []
        if is_dynamic_folder(selected):
            targets.append(selected)
        targets.extend(collect_dynamic_folders(selected))
        seen = set()
        result = []
    ...
    def _batch_add(self):
        ...
        if not parent:
            return
        folders = self._collect_dynamic_targets(parent)
        if not folders:
            messagebox.showinfo("提示", "所选目录及其子目录下没有找到同时包含 JPG 和 MP4 的动态照片文件夹")
            return
        added = self._add_common(folders)
    ...
    def _add_single(self):
        ...
        if not folder:
            return
        folders = self._collect_dynamic_targets(folder)
        if not folders:
