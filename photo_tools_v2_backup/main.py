"""main.py - PhotoTools V2.0 GUI 主界面"""
...
def load_config() -> dict:
    cfg = {"pushplus_token": "",
           "max_workers": 4, "appearance": "system"}
...
class ScoreViewFrame(ctk.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        # 右侧：评分细则面板
        self.detail_frame = ctk.CTkScrollableFrame(self, width=280)
        self.detail_frame.grid(row=0, column=1, sticky="nsew")
        self.detail_frame.configure(width=280)  # CTkScrollableFrame, fixed width

        ctk.CTkLabel(self.detail_frame, text="▸ 评分细则 ◂",
                      font=("", 14, "bold")).pack(pady=(10, 5))

        self.detail_text = ctk.CTkTextbox(self.detail_frame, width=260, height=400,
    ...
    def set_scale_detail(self, scale_key: str):
        self._scale_detail = build_scale_detail(scale_key)
    ...
    def add_scores(self, scores: List[PhotoScore], scale: str = "normal"):
        self.clear()
        self.set_scale_detail(scale)
        for s in scores:
            card = ctk.CTkFrame(self.scroll, corner_radius=8)
            card.pack(fill="x", padx=5, pady=4)
            ctk.CTkLabel(card, text=s.filename, font=("", 12, "bold"),
                          anchor="w").pack(anchor="w", padx=10, pady=(5, 2))
            info = ctk.CTkFrame(card, fg_color="transparent")
            info.pack(fill="x", padx=10, pady=2)
            color = "#2ecc71" if s.total_score >= 70 else (
                "#f39c12" if s.total_score >= 40 else "#e74c3c")
            ctk.CTkLabel(info, text=f"综合: {s.total_score:.1f}",
                          font=("", 14, "bold"), text_color=color).pack(side="left", padx=5)
            ctk.CTkLabel(info, text=f"构图: {s.composition:.0f}").pack(side="left", padx=4)
            ctk.CTkLabel(info, text=f"曝光: {s.exposure:.0f}").pack(side="left", padx=4)
            ctk.CTkLabel(info, text=f"清晰度: {s.sharpness:.0f}").pack(side="left", padx=4)
            ctk.CTkLabel(info, text=f"色彩: {s.color_score:.0f}").pack(side="left", padx=4)
            ctk.CTkLabel(info, text=f"噪点: {s.noise_score:.0f}").pack(side="left", padx=4)
            rc = "#2ecc71" if "优秀" in s.recommendation or "良好" in s.recommendation else "#f39c12"
            if s.recommendation:
                ctk.CTkLabel(card, text=f"建议: {s.recommendation}",
                              font=("", 11), text_color=rc,
...
class PhotoToolsApp(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("PhotoTools V2.0 — 摄影素材管理工具箱")
    ...
    def _build_ui(self):
        ...
        ctk.CTkLabel(ab, text="关于 PhotoTools V2.0", font=("", 14, "bold")).pack(anchor="w", pady=5)
        ctk.CTkLabel(ab, text="版本 2.0.0 | 摄影素材管理工具箱").pack(anchor="w")
                    ...
                    if ext in {".mp4", ".mov", ".avi", ".mkv"}:
                        ...
                        if vs:
                            scores.append(PhotoScore(
                                ...
                                total_score=vs.avg_score, recommendation=vs.recommendation))
                    else:
