...
from .installer import detect_first_install, is_installed
...
class PhotoToolsApp(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("PhotoTools V9.0 — 摄影素材管理工具箱")
    ...
    def _build_home(self):
        ...
        ctk.CTkLabel(header, text="PhotoTools V9.0", font=SF(26, "bold")).pack(anchor="w")
    ...
    def _build_ui(self):
        ...
        ctk.CTkButton(r7, text="一键安装", width=100, height=30,
                      font=("", 12), command=self._install_app).pack(side="left", padx=5)
        ctk.CTkButton(r7, text="一键卸载", width=100, height=30,
                      font=("", 12), fg_color="#c0392b", hover_color="#96281b",
                      command=self._uninstall_app).pack(side="left", padx=5)
        ...
        ctk.CTkLabel(ab, text="关于 PhotoTools V9.0", font=("", 14, "bold")).pack(anchor="w", pady=5)
        ctk.CTkLabel(ab, text="版本 9.0.0 | 摄影素材管理工具箱").pack(anchor="w")
    ...
    def _t1_preview(self,fp):
        from photo_tools_v9.preview import load_image
    ...
    def _install_app(self):
        ...
        ok, msg = install_app()
        if ok:
            messagebox.showinfo("安装完成", f"PhotoTools 已安装到:\n{msg}\n桌面已创建快捷方式")
        else:
            messagebox.showerror("安装失败", msg)
        self._refresh_install_status()

    def _uninstall_app(self):
        from .installer import uninstall_app
        if not messagebox.askyesno(
                "确认卸载",
                "将卸载 PhotoTools、删除桌面快捷方式并清理本机缓存？\n此操作不会删除你的照片素材。"):
            return
        errors = uninstall_app()
        if errors:
            messagebox.showwarning("卸载完成", "已卸载，但部分文件删除失败：\n" + "\n".join(errors))
        else:
            messagebox.showinfo("卸载完成", "PhotoTools 已卸载，缓存已清理")
        self._refresh_install_status()
