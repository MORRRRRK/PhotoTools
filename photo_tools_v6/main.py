...
class PhotoToolsApp(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("PhotoTools V6.0 — 摄影素材管理工具箱")
    ...
    def _build_ui(self):
        ...
        self.fm_opt = ctk.CTkOptionMenu(r0,values=["小","中","大","特大"],
            command=lambda v:setattr(self,"_font_scale",{"小":1.0,"中":1.25,"大":1.5,"特大":2.0}.get(v,1.0)))
        ...
        ctk.CTkLabel(ab, text="关于 PhotoTools V6.0", font=("", 14, "bold")).pack(anchor="w", pady=5)
        ctk.CTkLabel(ab, text="版本 6.0.0 | 摄影素材管理工具箱").pack(anchor="w")
        ctk.CTkLabel(ab, text="功能: 文件类型筛选 / 照片视频质量评估 / 视频代理生成 / 一键生成延时视频 / 动态照片提取 / PushPlus 推送").pack(anchor="w")
    ...
    def _t1_preview(self,fp):
        from photo_tools_v6.preview import load_image
