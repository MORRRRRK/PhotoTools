...
from .gallery_ui import GalleryTab
from .convert_ui import ConvertTab
from .gallery import clean_install_dir_cache
from .installer import get_setup_exe, get_uninstaller_exe, is_installed
...
# ===== 更新日志 =====
UPDATE_LOG = """V9.2（2026-08-23）
- 新增“RAW/PNG 转 JPG”：支持主流相机 RAW、PNG、TIFF 快速转换为 JPG
- 转换完成后显示生成的 JPG 文件位置，便于定位和删除审图缓存
- 支持批量转换、输出目录自定义、停止转换、已存在跳过

V9.1（2026-08-23）
- 新增独立安装器 PhotoToolsSetup.exe：支持自定义安装路径，安装主程序与卸载器
- 新增独立卸载器 PhotoToolsUninstall.exe：可选择保留本机缓存或彻底卸载
- 设置页“一键安装/一键卸载”改为启动独立安装器/卸载器
- 缓存改为安装目录下 PhotoTools\\preview_cache，二次安装到同一路径可复用

V9.0（2026-08-23）
- 新增“作品展示”：导入本地照片生成缩略图，点击查看大图并显示拍摄参数
- 左侧缩略图网格懒加载，支持大量图片；右侧显示分辨率、大小、格式、光圈、快门、ISO、焦距、设备、拍摄时间
- 预览图支持按钮与滚轮缩放；EXIF 缺失字段显示“未知”
- 缩略图与大图缓存只写入 %LOCALAPPDATA%\\PhotoTools\\preview_cache；首次安装时清理旧缓存，卸载时才清除全部缓存，日常使用保留缓存
- 设置页新增“一键安装”与“一键卸载”：安装到本机并注册系统卸载信息，卸载时自动清理缓存
...
class PhotoToolsApp(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("PhotoTools V9.2 — 摄影素材管理工具箱")
    ...
    def _build_home(self):
        ...
        ctk.CTkLabel(header, text="PhotoTools V9.2", font=SF(26, "bold")).pack(anchor="w")
        ...
        modules = [
            ...
            ("作品展示",
             "导入本地照片生成缩略图，点击查看大图与拍摄参数，缓存只保存在本机"),
            ("RAW/PNG 转 JPG",
             "把主流相机 RAW、PNG、TIFF 快速转为 JPG，显示生成位置方便审图后删除"),
            ("动态照片提取",
    ...
    def show_page(self, name):
        ...
        page.grid()
        if name == "作品展示":
            self.gallery_tab._schedule_render()
    ...
    def _build_ui(self):
        ...
        # ========== Tab: 作品展示 ==========
        t_gallery = make_page("作品展示")
        self.gallery_tab = GalleryTab(t_gallery, self)
        self.gallery_tab.pack(fill="both", expand=True, padx=10, pady=10)

        # ========== Tab: RAW/PNG 转 JPG ==========
        t_convert = make_page("RAW/PNG 转 JPG")
        self.convert_tab = ConvertTab(t_convert, self)
        ...
        r7 = ctk.CTkFrame(sf, fg_color="transparent")
        r7.pack(fill="x", pady=5)
        ctk.CTkLabel(r7, text="软件安装与卸载:", width=120, anchor="w").pack(side="left")
        self.install_status_lb = ctk.CTkLabel(r7, text="", width=80, anchor="w")
        self.install_status_lb.pack(side="left", padx=5)
        ctk.CTkButton(r7, text="打开安装程序", width=110, height=30,
                      font=("", 12), command=self._open_setup).pack(side="left", padx=5)
        ctk.CTkButton(r7, text="启动卸载器", width=100, height=30,
                      font=("", 12), fg_color="#c0392b", hover_color="#96281b",
                      command=self._launch_uninstaller).pack(side="left", padx=5)
        ...
        ctk.CTkLabel(ab, text="关于 PhotoTools V9.2", font=("", 14, "bold")).pack(anchor="w", pady=5)
        ctk.CTkLabel(ab, text="版本 9.2.0 | 摄影素材管理工具箱").pack(anchor="w")
        ctk.CTkLabel(ab, text="功能: 文件类型筛选 / 照片视频质量评估 / 视频代理生成 / 音频提取 / 一键生成延时视频 / 作品展示 / RAW/PNG 转 JPG / 动态照片提取 / PushPlus 推送").pack(anchor="w")
    ...
    def _t1_preview(self,fp):
        from photo_tools_v9_2.preview import load_image
    ...
    def _refresh_install_status(self):
        try:
            ...
            self.install_status_lb.configure(
                text="已安装" if installed else "未安装",
                text_color="#2ecc71" if installed else "#7f8c8d")
        except Exception:
            self.install_status_lb.configure(text="未知", text_color="#7f8c8d")

    def _open_setup(self):
        setup = get_setup_exe()
        if not setup or not os.path.exists(setup):
            messagebox.showerror("提示", "未找到 PhotoToolsSetup.exe，请先运行 build_installer.py 构建")
            return
        os.startfile(setup)

    def _launch_uninstaller(self):
        uninst = get_uninstaller_exe()
        if not uninst or not os.path.exists(uninst):
            messagebox.showerror("提示", "未找到 PhotoToolsUninstall.exe，请先运行 build_installer.py 构建")
            return
        os.startfile(uninst)
