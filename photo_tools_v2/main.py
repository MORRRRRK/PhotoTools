...
class ScoreViewFrame(ctk.CTkFrame):
    def __init__(self,master,**kwargs):
        ...
        ctk.CTkLabel(tf,text="\u7ec6\u5219",font=("",13,"bold")).grid(row=0,column=0,pady=(6,2))
        ...
        ctk.CTkLabel(self.preview_frame,text="\u9884\u89c8",font=("",13,"bold")).grid(row=0,column=0,pady=(6,4))
        self.preview_label=ctk.CTkLabel(self.preview_frame,text="\u70b9\u51fb\u5de6\u4fa7\u7ed3\u679c\u67e5\u770b\u9884\u89c8",font=("",12))
        self.preview_label.grid(row=1,column=0,sticky="nsew",padx=5,pady=(0,2))
        self.preview_info=ctk.CTkLabel(self.preview_frame,text="",font=("",10))
    ...
    def add_scores(self,scores,scale="normal"):
        ...
        for s in scores:
            ...
            fnl=ctk.CTkLabel(tr,text=os.path.basename(s.file)[:50],font=("",16,"bold"),anchor="w")
            fnl.pack(side="left",fill="x",expand=True)
            fnl.bind("<Button-1>",lambda e,fp=s.file:self._show_preview(fp))
            ctk.CTkButton(tr,text="X",width=26,height=26,fg_color="#c0392b",hover_color="#96281b",font=("",11,"bold"),
            ...
            ctk.CTkLabel(ai,text=" | ".join(parts),font=("",13)).pack(anchor="w")
            ctk.CTkLabel(ai,text=inf["path"],font=("",11),anchor="w").pack(anchor="w")
            ...
            ctk.CTkLabel(sc,text=f"{s.total_score:.0f}",font=("",18,"bold"),text_color=clr,width=40).pack(side="left")
            for lbl,val in[("\u6784",s.composition),("\u66dd",s.exposure),("\u6e05",s.sharpness),("\u8272",s.color_score),("\u566a",s.noise_score)]:
                ctk.CTkLabel(sc,text=f"{lbl}{val:.0f}",font=("",13),width=48).pack(side="left")
            if s.recommendation:
                rc="#2ecc71" if "\u4f18\u79c0"in s.recommendation or "\u826f\u597d"in s.recommendation else"#f39c12"
                ctk.CTkLabel(card,text=s.recommendation[:25],font=("",13),text_color=rc,anchor="w").pack(anchor="w",padx=12,pady=(0,4))
    ...
    def _show_preview(self,fp):
        ...
        if ext in ve:
            ...
            try:
                os.startfile(fp)
                self.preview_info.configure(text="\u5df2\u6253\u5f00\u9ed8\u8ba4\u64ad\u653e\u5668: "+os.path.basename(fp))
...
class PhotoToolsApp(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("PhotoTools V2.3 \u2014 \u6444\u5f71\u7d20\u6750\u7ba1\u7406\u5de5\u5177\u7bb1")
    ...
    def _build_ui(self):
        ...
        ctk.CTkLabel(ab,text="\u5173\u4e8e PhotoTools V2.3",font=("",14,"bold")).pack(anchor="w",pady=5)
