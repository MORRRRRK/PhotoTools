...
class QualityPageV101(QWidget):
    ...
    def _build_ui(self):
        ...
        header = QVBoxLayout()
        header.setSpacing(2)
        page_title = QLabel("照片质量评估")
        page_title.setObjectName("pageTitle")
        page_desc = QLabel("五维评分（构图 / 曝光 / 清晰度 / 色彩 / 噪点），按当前评分尺度给出保留建议。")
        page_desc.setObjectName("pageDesc")
        header.addWidget(page_title)
        header.addWidget(page_desc)
        center_lay.addLayout(header)
    ...
    def _build_info_panel(self):
        ...
        self.preview_lb = AspectImageView(placeholder="选择照片后显示预览")
        self.preview_lb.setMinimumHeight(260)
        lay.addWidget(self.preview_lb, 1)
    ...
    def _update_meta(self, path):
        ...
        if not self.preview_lb.set_image_path(path):
            self.preview_lb.clear_image("无法预览")
    ...
    def _reset_info(self):
        self.preview_lb.clear_image("选择照片后显示预览")
