...
MACOS_FIRST_RELEASE = {"单一文件类型筛选", "照片质量评估", "AI 图像增强", "作品展示", "设置", "关于"}


def filter_groups_for_platform(groups, platform_name: str):
    """macOS 首版只开放核心四项；其余模块在导航中隐藏。"""
    if platform_name != "darwin":
        return groups
    filtered = []
    for group_title, entries in groups:
        kept = [entry for entry in entries if entry[0] in MACOS_FIRST_RELEASE]
        if kept:
            filtered.append((group_title, kept))
    return filtered
...
class MainWindow(QMainWindow):
    ...
    def _build_central(self):
        ...
        groups = filter_groups_for_platform(groups, sys.platform)
