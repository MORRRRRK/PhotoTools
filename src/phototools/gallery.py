...
def get_install_path() -> str:
    ...
    try:
        if sys.platform != "win32":
            raise OSError("registry is windows only")
        import winreg
...
def read_image_info(path: str) -> dict:
    ...
    try:
        ...
        tags.update({str(k): str(v) for k, v in metadata.items()})
    except Exception:
        pass
    ...
    aperture = get("EXIF FNumber", "EXIF:FNumber", "FNumber")
    if aperture:
        info["aperture"] = f"f/{_ratio_text(aperture)}"
    shutter = get("EXIF ExposureTime", "EXIF:ExposureTime", "ExposureTime")
    if shutter:
        info["shutter"] = _shutter_text(shutter)
    iso = get("EXIF ISOSpeedRatings", "EXIF:ISO", "ISOSpeedRatings",
              "EXIF PhotographicSensitivity", "PhotographicSensitivity")
    if iso:
        info["iso"] = iso
    focal = get("EXIF FocalLength", "EXIF:FocalLength", "FocalLength")
    if focal:
        info["focal"] = f"{_ratio_text(focal)}mm"
    make = get("Image Make", "IFD0:Make", "EXIF:Make")
    model = get("Image Model", "IFD0:Model", "EXIF:Model")
    if make or model:
        info["device"] = f"{make or ''} {model or ''}".strip()
    taken = get("EXIF DateTimeOriginal", "EXIF:DateTimeOriginal", "DateTimeOriginal",
                "Image DateTime", "IFD0:DateTime", "EXIF DateTimeDigitized")
    if taken:
    ...
    try:
        ...
        if pyvips is not None:
            img = pyvips.Image.thumbnail(
                path, THUMB_SIZE[0] * 2, height=THUMB_SIZE[1] * 2)
...
def make_preview(path: str, cache_dir: str = None) -> str:
    ...
    try:
        ...
        if pyvips is not None:
            img = pyvips.Image.thumbnail(
                path, PREVIEW_MAX_SIZE[0], height=PREVIEW_MAX_SIZE[1])
            img.write_to_file(out)
            if os.path.exists(out) and os.path.getsize(out) > 0:
                return out
        with Image.open(path) as img:
