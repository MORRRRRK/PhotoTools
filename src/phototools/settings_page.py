...
class SettingsPage(QWidget):
    ...
    def _build_ui(self):
        ...
        if sys.platform == "darwin":
            uninstall, ulay, _ = _card("卸载", "删除应用本体、配置与缓存；不会删除你生成的图片和视频")
            self.uninstall_btn = QPushButton("卸载 PhotoTools")
            self.uninstall_btn.setProperty("danger", True)
            ulay.addWidget(self.uninstall_btn)
            root.addWidget(uninstall)
            self.uninstall_btn.clicked.connect(self._uninstall_macos)
    ...
    def _uninstall_macos(self):
        from .platform import install as platform_install
        from . import app_config as _cfg
        answer = QMessageBox.question(
            self, "确认卸载",
            "将把 PhotoTools.app 移入废纸篓，并删除配置、缓存与日志。\n"
            "你生成的图片和视频不会被删除。是否继续？")
        if answer != QMessageBox.Yes:
            return
        ok, message = platform_install.uninstall(remove_data=True)
        QMessageBox.information(self, "卸载", message) if ok else QMessageBox.warning(self, "卸载失败", message)
