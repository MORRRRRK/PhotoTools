...
from .platform.process import subprocess_flags

CREATE_NO_WINDOW = subprocess_flags()


def find_ffmpeg() -> Optional[str]:
    from .platform.ffmpeg import find_ffmpeg as _resolve_ffmpeg
    resolved = _resolve_ffmpeg()
    if resolved:
        return resolved
    legacy = PROJECT_ROOT / "assets" / ("ffmpeg.exe" if sys.platform == "win32" else "ffmpeg")
    if legacy.exists():
        return str(legacy)
    return None
